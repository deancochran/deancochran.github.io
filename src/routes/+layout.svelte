...
<!-- Header -->
<div
    class="flex h-full min-h-screen w-full flex-col items-start justify-between align-middle"
>
    <AppBar
    ...
    <!-- Content -->
    <div class="flex w-full grow flex-row align-middle">
        <aside class="transition-all duration-100 sm:w-1/4 lg:w-1/2"></aside>
        {#key data.pathname}
            <!-- Main -->
            <main
                class="flex w-full shrink flex-col gap-4 p-4"
                in:fade={{ duration: 200 }}
            >
                <ol class="flex items-center justify-start gap-2 text-xs">
                    {#each data.pathname.split('/').splice(1) as path, i}
                        {#if data.pathname.split('/').length > 2}
                            ...
                            {:else}
                                <li>
                                    <a
                                        class="capitalize opacity-60 hover:underline"
                                        href={`/${data.pathname.split('/').slice(1, i + 2)}`}
                                    >
                                        {path}
                                    </a>
                                </li>
                                <li class="opacity-60" aria-hidden="true">
                                    &rsaquo;
                                </li>
                            {/if}
                        {/if}
            ...
            <div class="flex w-full flex-row items-center gap-4">
                ...
                <div class="flex flex-col">
                    ...
                    <div class="flex w-full flex-row items-center gap-2">
                        <a href="https://www.linkedin.com/in/dean-cochran/"
                            ><Linkedin size={20} /></a
                        >
                        <a href="https://github.com/deancochran"
                            ><GithubIcon size={20} class="font-light" /></a
                        >
