...
> _While most of us are comfortable with using a microwave without understanding how it works, many do not feel the same way about AI yet, especially if that AI makes important decisions about their lives._

A common list of priorities that production system stake holders have:

- Latency and throughput speeds of the model
...
For the purposes of highlighting the key points of the authors work, I have listed out the _Key Requirements_ for ML systems in general
...
- Maintainability - Infrastructure as Code, Code Versioning, and other DevOps techniques for ML create the "MLOps" buzzword term. It is a niche overlap, but a requirement non the less
...
1. Scope out your project - outline aims. goals, constraints, evaluation criteria, budget, etc.

2. Engineer your Data - source datasets, develop pipelines, and process your data

3. Develop your model - go ahead with a model selection and training process, and find what is best for your project

4. Deploy your model - Make the model as accessible to end users/apps, evaluate which deployment technique is best for you

5. Check your model - evaluate the deployed model's performance, and evaluate the predictions for bias, fairness, etc.

6. Perform Business Analysis - finally, return to your goals and find the metrics that can offer business insights

## Data Engineering

A common feeling, the author finds, that engineers have is that tweaking the underlying ML algorithm in a clever way will make the model become drastically better. The author does not suggest ignoring the benefits of developing better algorithms, rather suggests that _instead of focusing on improving ML algorithms, a focus on managing and improving their data has shown to be much more beneficial for production models._

ML systems are rooted in data, while there are major improvements to model algorithms that are found in research state of the arts. The author also highlights throughout the text, that ultramodern models is not always made for production settings.

## Data sources

An ML System works with many different data sources. The author suggests sources may have different formats, characteristics, or even shared hidden relationships between other sourced data (multimodal relationships).
...
> _As the speed at which applications respond to users' queries has become a competitive advantage, it has become increasingly important to make data available for use as fast as possible._

As the author outlines, companies want online processing not just for transactional queries, but also for analytical queries. This is real-time data processing
...
Unstructured data is data that does not adhere to a predefined data schema. It is usually text but can also be numbers, dates, etc. For example, a text file of logs generated by your ML model is unstructured data.

Storage for structured data it called a **data warehouse**, and storage for unstructured data is called a \*\*data lake.

### Do you ETL or ELT

One of the more interesting sections in the text. The author does a wonderful job in highlighting some problems that companies really run into.

> With the knowledge of production data pipelines, the constantly changing formats of data, the rapid growth of the size of data, and the growth of data sources... _“Why not just store all data in a data lake so we do not have to deal with schema changes? Whichever application needs data can just pull-out raw data from there and process it.”_

**ETL** means to **Extract, Transform, and Load**. It can be general purpose processing and aggregating functions for manipulating data into the desired output shape. **ELT** means to **Extract, Load, and Transform.** Where loading data into storage first and then processing it later is used for faster arrival of formatted data

## Cost Sensitive Learning

One of the most interesting approaches the author discusses for handling class imbalance. Unlike traditional loss functions that treat all misclassifications equally, cost-sensitive learning assigns different costs to diverse types of errors.
...
The author makes an important distinction here that I found particularly valuable. While both approaches change the loss function, they serve different purposes:
...
This section resonated with me as it addresses a challenge in production ML systems. The author emphasizes that data distributions change over time, and models need to account for this reality.

Several types of distribution shifts:
...
> _In production, multiple types of shifts often happen simultaneously, making it difficult to find and address each type individually._

### Detecting Distribution Shifts

The author does a wonderful job breaking down various approaches to detect when your data has shifted. I particularly liked how she emphasizes that detecting shifts is not about mathematical tests.
...
> _While statistical tests can help detect distribution shifts, they should not be your only line of defense. Domain ability and business context are crucial for interpreting whether detected shifts are meaningful._
...
- Keeping separate models for different data distributions
...
The author presents a realistic view of model selection that I found refreshing. Instead of just chasing ultramodern performance, she emphasizes considering various practical factors:
...
> _A slightly less exact model that is faster, more interpretable, and easier to keep might be a better choice for your production system than an ultramodern model that requires significant computational resources._

I personally appreciated how the author keeps a practical perspective throughout this section, focusing on real-world considerations rather than just theoretical performance.
...
The author makes a fascinating point about how perturbation affects several types of data differently.

> _Adversarial augmentation is less common in NLP (an image of a bear with randomly added pixels still looks like a bear, but adding random characters to a random sentence will make it gibberish)_

This quote particularly resonated with me as it highlights a key difference between computer vision and NLP tasks that is not often discussed.

### Data Synthesis

**Notice: The author emphasizes that while data synthesis is an active research area, it is not yet widely adopted in production systems**

## Feature Engineering

This section was particularly valuable to me as it challenges a common misconception in the ML community. Chen makes a bold but correct statement:

> _State-of-the-art model architectures can still perform poorly if they do not use a good set of features._

### Learned vs. Engineered Features

The author addresses a question I have heard many times in my own work:

> _Why do we have to worry about feature engineering? Doesn't deep learning promise us that we no longer must engineer features?_

What I appreciate about Chen's response is her practical perspective. She acknowledges that while many features can be learned automatically, we are still far from complete automation. She gives a notable example with spam detection:

> _On NLP.... ML systems will need data beyond just what can be extracted from text. For example, when detecting whether a comment is spam or not, on top of the text in the comment itself, you might want to use other information about:_

> _- the comment: such as who posted this comment, how many upvotes/downvotes it has._

> _- the user who posted this comment: such as when this account was created, how often they post._

> _- the thread in which the comment was posted: such as how many views it has_
...
The section on scaling particularly resonated with me because it addresses a frequent problem in real-world data:

> _When we input these two variables into an ML model, it will not understand that 150,000 and 40 stand for different things. It will just see them both as numbers_
...
This section particularly stood out to me as it addresses a critical issue that is often overlooked in academic settings:
...
In _Deep Learning for Production_, Chen’s take on model development is pragmatic and direct—cutting through the noise to focus on the mechanics of building, tuning, and applying machine learning models effectively. Here is the essence of her insights:
...
- A core part is the **objective function** (or loss function), shaping how well a model performs. For classification, cross-entropy is common, while regression models often rely on Mean Squared Error. This aim defines a model's **loss surface**—the potential range of outcomes based on parameter values.
...
- Chen emphasizes that each ML task has a best model type, but starting simple is often best. Classification models handle binary, multiclass, or multilabel outputs, and certain tasks may receive help from hierarchical classification to simplify complex decisions.
...
- When juggling several goals, consider decoupling aim functions to keep clarity. Training separate models for each goal and then blending results provides flexibility without needing retraining.
...
Ensemble methods combine multiple models to improve predictive performance, particularly useful in handling complex data patterns and enhancing robustness. However, Chen points out that deploying ensembles in production is often challenging due to their complexity. Here is a breakdown of the primary types of ensemble methods:
...
Chen also recommends Kaggle’s _[Ensemble Guide](https://mlwave.com/kaggle-ensembling-guide)_, which offers further practical insights into implementing these techniques.
...
2. **Hard AutoML** (Architecture Search and Learned Optimizer):
   ...
   - **Concept**: Treats elements of model architecture (e.g., layer types, size) as hyperparameters. Instead of manually building the model architecture, a search algorithm can combine building blocks (e.g., convolution layers, pooling layers) to find best configurations.

   - **Neural Architecture Search (NAS)**: Consists of three parts:

     - **Search Space**: Defines network architectures.

     - **Performance Estimation**: Evaluates each candidate architecture.

     - **Search Strategy**: Guides the search for best architectures.
...
1. **Distributed Training**
   ...
   - **Purpose**: For large datasets that do not fit in memory or require high computational power, distributed training scales the process across multiple machines.
...
3. **Model Parallelism**
   ...
   - **Concept**: Unusual parts of the model are assigned to different machines. For example, one machine handles initial layers, while others handle next layers.
